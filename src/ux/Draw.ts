...
export class Draw {
    ...
    public static backg: HTMLImageElement = new Image()
    ...
    public static suppDial: HTMLImageElement = new Image()
    ...
    public static dia: HTMLImageElement = new Image()
    ...
    public static winScr: HTMLImageElement = new Image()
    ...
    public static images: HTMLElement[] = [Draw.lights, Draw.backg, Draw.safe, Draw.safeOpen, Draw.gold, 
    ...
    public static count: number = Draw.images.length
    ...
    public static behindSupp: ImageData
     ...
     */
    public static intersectButton(x, y){
        let inside = Math.sqrt((Draw.dim.centrDial[0]-x)**2 + (Draw.dim.centrDial[1]-y)**2) < Draw.dim.radiusSpin
        if(inside){
            ...
            Draw.spinWheel(Draw.currentRot, antiClockwise, -Arithmetic.degToRadians(360/9*Arithmetic.getRandomInt(9)), 
    ...
    public static drawImages(){
        ...
        if(Draw.initialDraw){
            ...
            Draw.behindSupp = Draw.ctx.getImageData(Draw.dim.ratios["supportDial"][0]*Draw.dim.width, 
                Draw.dim.ratios["supportDial"][1]*Draw.dim.height, Draw.suppDial.width, Draw.suppDial.height)
            Draw.behindInst = Draw.ctx.getImageData(Draw.dim.ratios["instructions"][0]*Draw.dim.width, 
                Draw.dim.ratios["instructions"][1]*Draw.dim.height, instructionsWidth, instructionsHeight)
            for (let i = 1; i <= 9; i++){
                Draw.behindSafes[`safe${i}`] = Draw.ctx.getImageData(Draw.dim.ratios[`safe${i}`][0]*Draw.dim.width,  
                    Draw.dim.ratios[`safe${i}`][1]*Draw.dim.height, Draw.safe.width, Draw.safe.height)
            }
        }
        ...
        for (let i = 1; i <= 9; i++){
            ...
            Draw.game.unlockedSafes.indexOf(i) === -1
                ? Draw.ctx.drawImage(Draw.safe, Draw.dim.ratios[s][0]*Draw.dim.width,  
                        Draw.dim.ratios[s][1]*Draw.dim.height, widthFactor, heightFactor)
                : Draw.openSafe(i)
        }


        if(Draw.game.state !== Game.states["WON"]){
            Draw.ctx.drawImage(Draw.suppDial, Draw.dim.ratios["supportDial"][0]*Draw.dim.width,  
                Draw.dim.ratios["supportDial"][1]*Draw.dim.height, Draw.suppDial.width*Draw.dim.shrink, 
                Draw.suppDial.height*Draw.dim.shrink)
            Draw.ctx.drawImage(Draw.dial, 0, 0, Draw.dim.thirdDialW, Draw.dial.height, 
                Draw.dim.ratios["dial"][0]*Draw.dim.width,  Draw.dim.ratios["dial"][1]*Draw.dim.height, 
                Draw.dim.thirdDialW*Draw.dim.shrink, Draw.dial.height*Draw.dim.shrink)
        }

        if(Draw.initialDraw){
            Draw.behindLight2 = Draw.ctx.getImageData(Draw.dim.ratios["lights2"][0]*Draw.dim.width,  
                Draw.dim.ratios["lights2"][1]*Draw.dim.height, Draw.dim.thirdLightsW, Draw.lights.height)
            Draw.behindLight1 = Draw.ctx.getImageData(Draw.dim.ratios["lights1"][0]*Draw.dim.width,  
                Draw.dim.ratios["lights1"][1]*Draw.dim.height, Draw.dim.thirdLightsW, Draw.lights.height)    
            Draw.behindSpin = Draw.ctx.getImageData(Draw.dim.ratios["spin"][0]*Draw.dim.width,  
                Draw.dim.ratios["spin"][1]*Draw.dim.height, Draw.spin.width, Draw.spin.height)
            Draw.behindMarker = Draw.ctx.getImageData(Draw.dim.ratios["marker"][0]*Draw.dim.width,  
                Draw.dim.ratios["marker"][1]*Draw.dim.height, Draw.marker.width/2, Draw.marker.height)
        }

        if(Draw.game.state !== Game.states["WON"]){
            Draw.ctx.drawImage(Draw.spin, Draw.dim.ratios["spin"][0]*Draw.dim.width,  
                Draw.dim.ratios["spin"][1]*Draw.dim.height, Draw.spin.width*Draw.dim.shrink, 
                Draw.spin.height*Draw.dim.shrink)
            Draw.ctx.drawImage(Draw.lights, 0, 0, Draw.dim.thirdLightsW, Draw.lights.height, 
                Draw.dim.ratios["lights1"][0]*Draw.dim.width,  Draw.dim.ratios["lights1"][1]*Draw.dim.height, 
                Draw.dim.thirdLightsW*Draw.dim.shrink, Draw.lights.height*Draw.dim.shrink)
            Draw.ctx.drawImage(Draw.lights, Draw.dim.thirdLightsW, 0, Draw.dim.thirdLightsW, Draw.lights.height, 
                Draw.dim.ratios["lights2"][0]*Draw.dim.width,  Draw.dim.ratios["lights2"][1]*Draw.dim.height, 
                Draw.dim.thirdLightsW*Draw.dim.shrink, Draw.lights.height*Draw.dim.shrink)
        }

        if(Draw.fontsLoaded) {Draw.writeWords()} 

        if(Draw.resizing || Draw.initialDraw) {
            // Sets dimensions with respect to the browser size
            Draw.dim.setDimensions(Draw.suppDial.width, Draw.suppDial.height, Draw.dial.width, 
                Draw.dial.height, Draw.spin.width)
    ...
    public static drawStars(move){
            ...
            if (Draw.stars[i]) {
                if (Draw.stars[i].distanceFromSource >= Dim.maxStarDistance) {
                    ...
                    if(move){
                        ...
                        Draw.stars[i].distanceFromSource = Math.sqrt((Draw.stars[i].x - Draw.stars[i].source[0])**2 + (Draw.stars[i].y - Draw.stars[i].source[1])**2)
                    }
        ...
        Draw.ctx.drawImage(image, Draw.dim.winImageSX, 0, image.width/2, image.height, s1x-image.width*Draw.dim.shrink*(scale-1)/4, 
        s1y-image.height*Draw.dim.shrink*(scale-1)/2, scale*image.width*Draw.dim.shrink/2, scale*image.height*Draw.dim.shrink)

        Draw.ctx.drawImage(image, Draw.dim.winImageSX, 0, image.width/2, image.height, s2x-image.width*Draw.dim.shrink*(scale-1)/4, 
        s2y-image.height*Draw.dim.shrink*(scale-1)/2, scale*image.width*Draw.dim.shrink/2, scale*image.height*Draw.dim.shrink)
    ...
    public static writeWords() {
        switch (Draw.game.state) {
            case Game.states["ZERO_SPINS"] :{
                ...
                Draw.drawScreen()
                Draw.ctx.fillText('Match a pair of symbols for a safe busting multiplier!', 
                    Draw.dim.ratios["instructionsTop"][0]*Draw.dim.width, 
                    Draw.dim.ratios["instructionsTop"][1]*Draw.dim.height)
                Draw.ctx.fillText('TOUCH THE DIAL TO SPIN YOUR 4 DIGIT COMBINATION', 
                    Draw.dim.ratios["instructionsBottom"][0]*Draw.dim.width, 
                    Draw.dim.ratios["instructionsBottom"][1]*Draw.dim.height)
                Draw.ctx.font = `${Dim.instFontSize*Draw.dim.shrink}px unlocked`
                Draw.ctx.fillText("-   -   -   -", Draw.dim.ratios["unlockedSafes"][0]*Draw.dim.width, 
                    Draw.dim.ratios["unlockedSafes"][1]*Draw.dim.height)
                break
            }
            case Game.states["SPINNING"] :{
                Draw.ctx.font = `${Dim.headrFontSize*Draw.dim.shrink}px instructions`                
                Draw.drawBehindInst()
                Draw.ctx.fillText('SPINNING!', Draw.dim.ratios["spinning"][0]*Draw.dim.width, 
                    Draw.dim.ratios["spinning"][1]*Draw.dim.height)
                break
            }
            ...
            case Game.states["WON"] :{
                Draw.ctx.drawImage(Draw.winScr, Draw.dim.ratios["winScreen"][0]*Draw.dim.width,  
                    Draw.dim.ratios["winScreen"][1]*Draw.dim.height, Draw.winScr.width*Draw.dim.shrink, 
                    Draw.winScr.height*Draw.dim.shrink)
                Draw.ctx.font = `${Dim.winScrnFontSize*Draw.dim.shrink}px unlocked`
                Draw.ctx.fillText("WIN", Draw.dim.ratios["winText"][0]*Draw.dim.width, 
                    Draw.dim.ratios["winText"][1]*Draw.dim.height)
                Draw.ctx.font = `${Dim.headrFontSize*Draw.dim.shrink}px instructions`
                let amountWon = Draw.game.boxes[Draw.game.winSafes[0]]*Game.bet
                Draw.ctx.fillText(`YOU WIN £${amountWon}!`, Draw.dim.ratios["youWinText"][0]*Draw.dim.width, 
                    Draw.dim.ratios["youWinText"][1]*Draw.dim.height)
                break
            }
            case Game.states["LOST"] :{
                ...
                Draw.ctx.fillText(`Click to replay`, Draw.dim.ratios["replayText"][0]*Draw.dim.width, 
                    Draw.dim.ratios["replayText"][1]*Draw.dim.height)
                Draw.ctx.font = `${Dim.headrFontSize*Draw.dim.shrink}px instructions`
                Draw.ctx.fillText(`NO LUCK THIS TIME!`, Draw.dim.ratios["noLuckText"][0]*Draw.dim.width, 
                    Draw.dim.ratios["noLuckText"][1]*Draw.dim.height)
                break
            }
        }
    }
     ...
     */
    public static writeSPUNstate(){
        Draw.drawBehindInst()
        Draw.ctx.font = `${Dim.headrFontSize*Draw.dim.shrink}px instructions`
        Draw.ctx.fillText("SAFE" + Draw.game.result.toString(), Draw.dim.ratios["safeText"][0]*Draw.dim.width, 
            Draw.dim.ratios["safeText"][1]*Draw.dim.height)
        Draw.drawScreen()
        Draw.ctx.font = `${Dim.instFontSize*Draw.dim.shrink}px unlocked`
        Draw.ctx.fillText(Draw.game.getUnlockedSafesString(), Draw.dim.ratios["unlockedSafes"][0]*Draw.dim.width, 
            Draw.dim.ratios["unlockedSafes"][1]*Draw.dim.height)
    }
    ...
    public static drawScreen(){
        Draw.ctx.drawImage(Draw.screen, Draw.dim.ratios["screen"][0]*Draw.dim.width,  
            Draw.dim.ratios["screen"][1]*Draw.dim.height, Draw.screen.width*Draw.dim.shrink, 
            Draw.screen.height*Draw.dim.shrink)
    }

    /**
     * Helper method
     */
    public static drawBehindInst(){
        Draw.ctx.putImageData(Draw.behindInst, Draw.dim.ratios["instructions"][0]*Draw.dim.width, 
            Draw.dim.ratios["instructions"][1]*Draw.dim.height, 0, 0, Draw.behindInst.width*Draw.dim.shrink, 
            Draw.behindInst.height*Draw.dim.shrink)
    }
    ...
    public static openSafe(result){
        ...
        Draw.ctx.putImageData(Draw.behindSafes[s], Draw.dim.ratios[s][0]*Draw.dim.width, Draw.dim.ratios[s][1]*Draw.dim.height,
            0, 0, Draw.behindSafes[s].width*Draw.dim.shrink, Draw.behindSafes[s].height*Draw.dim.shrink)

        // Draw open safe
        Draw.ctx.drawImage(Draw.safeOpen, Draw.dim.ratios[s][0]*Draw.dim.width + Dim.openSafeXTrans*Draw.dim.shrink,  
            Draw.dim.ratios[s][1]*Draw.dim.height + Dim.openSafeYTrans*Draw.dim.shrink, 
            Draw.safeOpen.width*Draw.dim.shrink, Draw.safeOpen.height*Draw.dim.shrink)
    ...
    public static drawLights(){
        Draw.ctx.putImageData(Draw.behindLight1, Draw.dim.ratios["lights1"][0]*Draw.dim.width, 
            Draw.dim.ratios["lights1"][1]*Draw.dim.height, 0, 0, Draw.behindLight1.width*Draw.dim.shrink, 
            Draw.behindLight1.height*Draw.dim.shrink)
        Draw.ctx.putImageData(Draw.behindLight2, Draw.dim.ratios["lights2"][0]*Draw.dim.width, 
            Draw.dim.ratios["lights2"][1]*Draw.dim.height, 0, 0, Draw.behindLight2.width*Draw.dim.shrink, 
            Draw.behindLight2.height*Draw.dim.shrink)

        Draw.ctx.drawImage(Draw.lights, Draw.dim.xLights1*Draw.dim.thirdLightsW, 0, Draw.dim.thirdLightsW, 
            Draw.lights.height, Draw.dim.ratios["lights1"][0]*Draw.dim.width,  
            Draw.dim.ratios["lights1"][1]*Draw.dim.height, Draw.dim.thirdLightsW*Draw.dim.shrink, 
            Draw.lights.height*Draw.dim.shrink)
        Draw.ctx.drawImage(Draw.lights, Draw.dim.xLights2*Draw.dim.thirdLightsW, 0, Draw.dim.thirdLightsW, 
            Draw.lights.height, Draw.dim.ratios["lights2"][0]*Draw.dim.width,  
            Draw.dim.ratios["lights2"][1]*Draw.dim.height, Draw.dim.thirdLightsW*Draw.dim.shrink, 
            Draw.lights.height*Draw.dim.shrink)
    }
    ...
    public static rotate(rotation, xTranslate){
        ...
        Draw.ctx.drawImage(Draw.dial, xTranslate, 0, Draw.dim.thirdDialW, Draw.dial.height, 
            Draw.dim.ratios["dial"][0]*Draw.dim.width-Draw.dim.centrDial[0],  
            Draw.dim.ratios["dial"][1]*Draw.dim.height-Draw.dim.centrDial[1], 
            Draw.dim.thirdDialW*Draw.dim.shrink, Draw.dial.height*Draw.dim.shrink) 
        Draw.ctx.setTransform(1,0,0,1,0,0)
        if(Draw.spinOn){
            Draw.ctx.drawImage(Draw.spin, Draw.dim.ratios["spin"][0]*Draw.dim.width,  
                Draw.dim.ratios["spin"][1]*Draw.dim.height, Draw.spin.width*Draw.dim.shrink, Draw.spin.height*Draw.dim.shrink)
        }
    }
    ...
    public static async generateSparks() {
        await Promise.all([Draw.dim.getPoint(), Draw.dim.getPoint(), Draw.dim.getPoint(), Draw.dim.getPoint(), 
            ...
            for (let i = 0; i < Draw.sparks.length; i++){ 
                if (Draw.sparks[i]){
                    if (set.has(i)) {
                        ...
                        Draw.ctx.drawImage(Draw.sparkSafe, Draw.sparks[i].x-Draw.sparks[i].size*Draw.dim.shrink/2, 
                            Draw.sparks[i].y-Draw.sparks[i].size*Draw.dim.shrink/2, Draw.sparks[i].size*Draw.dim.shrink, Draw.sparks[i].size*Draw.dim.shrink)
                    }
                    else {
                        Draw.ctx.drawImage(Draw.sparkSafe, Draw.sparks[i].x-Draw.sparks[i].size*Draw.dim.shrink/2, 
                            Draw.sparks[i].y-Draw.sparks[i].size*Draw.dim.shrink/2, Draw.sparks[i].size*Draw.dim.shrink, Draw.sparks[i].size*Draw.dim.shrink)
    ...
    public static flashSpin(){
        Draw.ctx.putImageData(Draw.behindSpin, Draw.dim.ratios["spin"][0]*Draw.dim.width, 
            Draw.dim.ratios["spin"][1]*Draw.dim.height, 0, 0, Draw.behindSpin.width*Draw.dim.shrink, 
            Draw.behindSpin.height*Draw.dim.shrink)
        if(Draw.spinOn){
            Draw.spinOn = false
        }
        else {
    ...
    public static redDial(counter){
        if (counter == 10){
            ...
            if (counter % 2 == 0) {
                ...
                Draw.ctx.drawImage(Draw.marker, 0, 0, Draw.marker.width/2, Draw.marker.height, 
                    Draw.dim.ratios["marker"][0]*Draw.dim.width,  Draw.dim.ratios["marker"][1]*Draw.dim.height, 
                    Draw.marker.width*Draw.dim.shrink/2, Draw.marker.height*Draw.dim.shrink)
            }
            else {
                Draw.ctx.putImageData(Draw.behindMarker, Draw.dim.ratios["marker"][0]*Draw.dim.width, 
                    Draw.dim.ratios["marker"][1]*Draw.dim.height, 0, 0, Draw.behindMarker.width*Draw.dim.shrink, 
                    Draw.behindMarker.height*Draw.dim.shrink)
                Draw.rotate(Draw.currentRot, 0)
            }
        counter ++
    ...
    public static implementLoss(){
        ...
        Draw.ctx.drawImage(Draw.panelBackg, Draw.dim.ratios["panelBackground"][0]*Draw.dim.width,  
            Draw.dim.ratios["panelBackground"][1]*Draw.dim.height, 
            Draw.panelBackg.width*Draw.dim.shrink, 
            Draw.panelBackg.height*Draw.dim.shrink)
        Draw.ctx.drawImage(Draw.screenBackg, Draw.dim.ratios["screenBackground"][0]*Draw.dim.width,  
            Draw.dim.ratios["screenBackground"][1]*Draw.dim.height, Draw.screenBackg.width*Draw.dim.shrink, 
            Draw.screenBackg.height*Draw.dim.shrink)
