...
export class Draw {
    ...
    public static drawImages(){
        ...
        if(Draw.initialDraw){
            ...
            for (let i = 1; i <= 9; i++){
                Draw.behindSafes[`safe${i}`] = Draw.ctx.getImageData(Draw.dim.ratios[`safe${i}`][0]*Draw.dim.width,  
                    Draw.dim.ratios[`safe${i}`][1]*Draw.dim.height, widthFactor, heightFactor)
            }
        }
        ...
        for (let i = 1; i <= 9; i++){
            ...
            Draw.game.unlockedSafes.indexOf(i) === -1
                ? Draw.ctx.drawImage(this.safe, Draw.dim.ratios[s][0]*Draw.dim.width,  
        ...
        if(Draw.game.state !== Game.states["WON"]){
            Draw.ctx.drawImage(this.suppDial, Draw.dim.ratios["supportDial"][0]*Draw.dim.width,  
                Draw.dim.ratios["supportDial"][1]*Draw.dim.height, this.suppDial.width*Draw.dim.shrink, 
                this.suppDial.height*Draw.dim.shrink)
            Draw.ctx.drawImage(this.dial, 0, 0, Draw.dim.thirdDialW, this.dial.height, 
                Draw.dim.ratios["dial"][0]*Draw.dim.width,  Draw.dim.ratios["dial"][1]*Draw.dim.height, 
                Draw.dim.thirdDialW*Draw.dim.shrink, this.dial.height*Draw.dim.shrink)
        }

        if(Draw.initialDraw){
            Draw.behindLight2 = Draw.ctx.getImageData(Draw.dim.ratios["lights2"][0]*Draw.dim.width,  
                Draw.dim.ratios["lights2"][1]*Draw.dim.height, Draw.dim.thirdLightsW, this.lights.height)
            Draw.behindLight1 = Draw.ctx.getImageData(Draw.dim.ratios["lights1"][0]*Draw.dim.width,  
                Draw.dim.ratios["lights1"][1]*Draw.dim.height, Draw.dim.thirdLightsW, this.lights.height)    
            Draw.behindSpin = Draw.ctx.getImageData(Draw.dim.ratios["spin"][0]*Draw.dim.width,  
                Draw.dim.ratios["spin"][1]*Draw.dim.height, this.spin.width, this.spin.height)
        ...
        if(Draw.game.state !== Game.states["WON"]){
            Draw.ctx.drawImage(this.spin, Draw.dim.ratios["spin"][0]*Draw.dim.width,  
                Draw.dim.ratios["spin"][1]*Draw.dim.height, this.spin.width*Draw.dim.shrink, 
                this.spin.height*Draw.dim.shrink)
            Draw.ctx.drawImage(this.lights, 0, 0, Draw.dim.thirdLightsW, this.lights.height, 
                Draw.dim.ratios["lights1"][0]*Draw.dim.width,  Draw.dim.ratios["lights1"][1]*Draw.dim.height, 
                Draw.dim.thirdLightsW*Draw.dim.shrink, this.lights.height*Draw.dim.shrink)
            Draw.ctx.drawImage(this.lights, Draw.dim.thirdLightsW, 0, Draw.dim.thirdLightsW, this.lights.height, 
                Draw.dim.ratios["lights2"][0]*Draw.dim.width,  Draw.dim.ratios["lights2"][1]*Draw.dim.height, 
                Draw.dim.thirdLightsW*Draw.dim.shrink, this.lights.height*Draw.dim.shrink)
        }

        if(Draw.fontsLoaded) {Draw.writeWords(Dim.instFontSize)} 
    ...
    public static drawStars(move){
        ...
        Draw.ctx.drawImage(image, Draw.dim.winImageSX, 0, image.width/2, image.height, s1x-image.width*(scale-1)/4, 
        s1y-image.height*(scale-1)/2, scale*image.width*Draw.dim.shrink/2, scale*image.height*Draw.dim.shrink)

        Draw.ctx.drawImage(image, Draw.dim.winImageSX, 0, image.width/2, image.height, s2x-image.width*(scale-1)/4, 
        s2y-image.height*(scale-1)/2, scale*image.width*Draw.dim.shrink/2, scale*image.height*Draw.dim.shrink)
    ...
    public static writeWords(fSize) {
        ...
        switch (Draw.game.state) {
            ...
            case Game.states["WON"] :{
                ...
                Draw.ctx.fillText(`YOU WIN £${amountWon}!`, Draw.dim.ratios["safeText"][0]*Draw.dim.width, 
     ...
     */
    public static writeSPUNstate(fontSize){
        Draw.drawBehindInst()
        Draw.ctx.font = `${fontSize*Draw.dim.shrink}px instructions`
    ...
    public static rotate(rotation, xTranslate){
        ...
        Draw.ctx.drawImage(this.dial, xTranslate, 0, Draw.dim.thirdDialW, this.dial.height, 
        ...
        if(Draw.spinOn){
            Draw.ctx.drawImage(this.spin, Draw.dim.ratios["spin"][0]*Draw.dim.width,  
                Draw.dim.ratios["spin"][1]*Draw.dim.height, this.spin.width*Draw.dim.shrink, this.spin.height*Draw.dim.shrink)
        }
    }
